...
def plot_confusion_matrix(y_true, y_pred):
    ...
    plt.xlabel('Predicho')
    plt.tight_layout()
    plt.savefig('algoritmos/resultados/confusion_matrix_svm.png')  #Guardamos la imagen en la carpeta "resultados".
    plt.show()
...
def entrenar_y_evaluar_modelo(X_train_scaled, X_test_scaled, y_train, y_test):
    #Entrenamos el modelo de SVM usando GridSearchCV y evaluamos su rendimiento.
    
    #Definimos los parámetros para la búsqueda.
    param_grid = {
        'C': [0.1, 1], #Aplicamos valores más restrictivos de regularización.
        'gamma': ['scale'], #Trata de controlar la forma de la función.
        'kernel': ['rbf'] #Se trata del Kernel del modelo SVM. "El rbf", introduce no linealidades al modelo.
        #Trata de proyectar los datos en un espacio de mayor dimensión. (Adecuado para detectar anomalías).
    }
    
    # Búsqueda de mejores parámetros
    grid_search = GridSearchCV(
        SVC(    #Realizamos el algoritmo Support Vector Machine (Classifier).
            ...
            cache_size=1000          # Para que haya mejor rendimiento.
        ),
...
def main():
    #Antes de comenzar a realizar el modelo de SVM, creamos un directorio para resultados si no existe llamado "resultados".
    ...
    try:
        ...
        with open('algoritmos/resultados/svm_results.txt', 'w', encoding='utf-8') as f:
